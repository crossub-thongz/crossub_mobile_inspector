#!/usr/bin/env bash
# Frees the inspector dev port and starts the app. Default port 3006 so it can
# run alongside crossub_web (web :3000, api :3001), agent (:3002), tenant (:3003),
# maintenance (:3004), landlord (:3005).
set -euo pipefail
...
if [ -d "$ROOT/apps/inspector/.next" ]; then
  echo "→ Clearing stale .next cache"
  rm -rf "$ROOT/apps/inspector/.next"
fi
...
echo "→ Starting inspector app on port $PORT (API proxy → crossub_web @ :3001)"
